...
#ifdef COMM_SHMWIN
   // Shared-memory-window halo exchange: commDataRecv lives in an
   // MPI_Win_allocate_shared segment; peerRecv[r] is rank r's recv buffer
   // mapped into this process, so sends are direct cudaMemcpys into it.
   MPI_Comm shmComm ;
   MPI_Win  shmWin ;
   Real_t **peerRecv ;
#endif
...
#ifdef COMM_IPC
   // CUDA-IPC halo exchange: d_peerRecv[r] is rank r's d_commDataRecv
   // (GPU memory) mapped into this process via cudaIpcOpenMemHandle, so
   // sends are direct device-to-device cudaMemcpys into the peer's GPU.
   Real_t **d_peerRecv ;
#ifdef IPC_VIA_MPIWRAP
   // Peer pointers come from MPI_Win_create + MPI_Win_shared_query instead
   // of an explicit handle exchange; libmpiwrap.so (LD_PRELOAD) implements
   // the window over device memory with CUDA IPC.
   MPI_Win ipcWin ;
#endif
#endif
#endif
