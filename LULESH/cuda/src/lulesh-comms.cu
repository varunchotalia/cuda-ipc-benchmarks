...
void CommRecv(Domain& domain, int msgType, Index_t xferFields,
   ...
   if (planeMin && doRecv) {
      ...
      MPI_Irecv(&recvBuf[pmsg * maxPlaneComm],
              ...
              Index_t dx, Index_t dy, Index_t dz, bool doSend, bool planeOnly)
{
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
         destAddr = COMM_HOST_SEND_DEST(domain, myRank - domain.tp()*domain.tp(), 0, 0, 1,
            &domain.commDataSend[pmsg * maxPlaneComm], xferFields, doSend, planeOnly) ;
         ...
         COMM_HOST_ISEND(destAddr, xferFields*sendCount, baseType, myRank - domain.tp()*domain.tp(), msgType, &domain.sendRequest[pmsg]) ;
         ++pmsg ;
      }
      if (planeMax && doSend) {
         destAddr = COMM_HOST_SEND_DEST(domain, myRank + domain.tp()*domain.tp(), 0, 0, -1,
   ...
   if (rowMin | rowMax) {
      ...
      if (rowMin) {
         destAddr = COMM_HOST_SEND_DEST(domain, myRank - domain.tp(), 0, 1, 0,
            &domain.commDataSend[pmsg * maxPlaneComm], xferFields, doSend, planeOnly) ;
         for (Index_t fi=0; fi<xferFields; ++fi) {
            Domain_member src = fieldData[fi] ;
         ...
         COMM_HOST_ISEND(destAddr, xferFields*sendCount, baseType, myRank - domain.tp(), msgType, &domain.sendRequest[pmsg]) ;
         ++pmsg ;
      }
      if (rowMax && doSend) {
         destAddr = COMM_HOST_SEND_DEST(domain, myRank + domain.tp(), 0, -1, 0,
   ...
   if (colMin | colMax) {
      ...
      if (colMin) {
         destAddr = COMM_HOST_SEND_DEST(domain, myRank - 1, 1, 0, 0,
            &domain.commDataSend[pmsg * maxPlaneComm], xferFields, doSend, planeOnly) ;
         for (Index_t fi=0; fi<xferFields; ++fi) {
            Domain_member src = fieldData[fi] ;
            for (Index_t i=0; i<dz; ++i) {
         ...
         COMM_HOST_ISEND(destAddr, xferFields*sendCount, baseType, myRank - 1, msgType, &domain.sendRequest[pmsg]) ;
         ++pmsg ;
      }
      if (colMax && doSend) {
         destAddr = COMM_HOST_SEND_DEST(domain, myRank + 1, -1, 0, 0,
   ...
   if (!planeOnly) {
      if (rowMin && colMin) {
         ...
         destAddr = COMM_HOST_SEND_DEST(domain, toRank, 1, 1, 0,
            &domain.commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm], xferFields, doSend, planeOnly) ;
         for (Index_t fi=0; fi<xferFields; ++fi) {
            Domain_member src = fieldData[fi] ;
            for (Index_t i=0; i<dz; ++i) {
         ...
         COMM_HOST_ISEND(destAddr, xferFields*dz, baseType, toRank, msgType, &domain.sendRequest[pmsg+emsg]) ;
         ++emsg ;
      }

      if (rowMin && planeMin) {
         int toRank = myRank - domain.tp()*domain.tp() - domain.tp() ;
         destAddr = COMM_HOST_SEND_DEST(domain, toRank, 0, 1, 1,
            &domain.commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm], xferFields, doSend, planeOnly) ;
         for (Index_t fi=0; fi<xferFields; ++fi) {
            Domain_member src = fieldData[fi] ;
         ...
         COMM_HOST_ISEND(destAddr, xferFields*dx, baseType, toRank, msgType, &domain.sendRequest[pmsg+emsg]) ;
         ++emsg ;
      }

      if (colMin && planeMin) {
         int toRank = myRank - domain.tp()*domain.tp() - 1 ;
         destAddr = COMM_HOST_SEND_DEST(domain, toRank, 1, 0, 1,
            &domain.commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm], xferFields, doSend, planeOnly) ;
         for (Index_t fi=0; fi<xferFields; ++fi) {
            Domain_member src = fieldData[fi] ;
         ...
         COMM_HOST_ISEND(destAddr, xferFields*dy, baseType, toRank, msgType, &domain.sendRequest[pmsg+emsg]) ;
         ++emsg ;
      }
      ...
      if (rowMin && colMin && planeMin) {
         ...
         Real_t *comBuf = COMM_HOST_SEND_DEST(domain, toRank, 1, 1, 1,
            &domain.commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm + cmsg * CACHE_COHERENCE_PAD_REAL], xferFields, doSend, planeOnly) ;
         for (Index_t fi=0; fi<xferFields; ++fi) {
            comBuf[fi] = (domain.*fieldData[fi])(0) ;
         }
         COMM_HOST_ISEND(comBuf, xferFields, baseType, toRank, msgType, &domain.sendRequest[pmsg+emsg+cmsg]) ;
         ++cmsg ;
      }
   ...
   COMM_HOST_SEND_FINISH(domain, status) ;
}
