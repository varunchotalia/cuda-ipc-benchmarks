...
/* Pointer-taking variants for backends whose halo data arrives in device
              ...
              Index_t dx, Index_t dy, Index_t dz, bool doSend, bool planeOnly, cudaStream_t stream)
{
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
	    ...
	    SendPlane<0><<<(sendCount+block-1)/block,block,0,stream>>>(d_destAddr, &(domain.*src)(0), sendCount, dx, dy, dz);
         ...
         COMM_SEND_MSG(domain, myRank - domain.tp()*domain.tp(), 0, 0, 1,
                       d_destAddr, &domain.commDataSend[pmsg * maxPlaneComm],
                       xferFields*sendCount, baseType, msgType,
                       &domain.sendRequest[pmsg], stream, xferFields, doSend, planeOnly) ;
         ++pmsg ;
      }
      if (planeMax && doSend) {
         d_destAddr = COMM_PACK_DEST(domain, myRank + domain.tp()*domain.tp(), 0, 0, -1,
   ...
   if (rowMin | rowMax) {
      ...
      if (rowMin) {
	    ...
	    SendPlane<2><<<(sendCount+block-1)/block,block,0,stream>>>(d_destAddr, &(domain.*src)(0), sendCount, dx, dy, dz);
         ...
         d_destAddr -= xferFields*sendCount ;
         COMM_SEND_MSG(domain, myRank - domain.tp(), 0, 1, 0,
                       d_destAddr, &domain.commDataSend[pmsg * maxPlaneComm],
                       xferFields*sendCount, baseType, msgType,
                       &domain.sendRequest[pmsg], stream, xferFields, doSend, planeOnly) ;
         ++pmsg ;
   ...
   if (colMin | colMax) {
      ...
      if (colMin) {
	    ...
	    SendPlane<4><<<(sendCount+block-1)/block,block,0,stream>>>(d_destAddr, &(domain.*src)(0), sendCount, dx, dy, dz);
         ...
         d_destAddr -= xferFields*sendCount ;
         COMM_SEND_MSG(domain, myRank - 1, 1, 0, 0,
                       d_destAddr, &domain.commDataSend[pmsg * maxPlaneComm],
                       xferFields*sendCount, baseType, msgType,
                       &domain.sendRequest[pmsg], stream, xferFields, doSend, planeOnly) ;
         ++pmsg ;
   ...
   if (!planeOnly) {
      if (rowMin && colMin) {
	    ...
	    SendEdge<0><<<(dz+block-1)/block,block,0,stream>>>(d_destAddr, &(domain.*src)(0), dz, dx, dy, dz);
         ...
         COMM_SEND_MSG(domain, toRank, 1, 1, 0,
                       d_destAddr, &domain.commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm],
                       xferFields*dz, baseType, msgType,
                       &domain.sendRequest[pmsg+emsg], stream, xferFields, doSend, planeOnly) ;
         ++emsg ;
      }
      ...
      if (rowMin && planeMin) {
	    ...
	    SendEdge<1><<<(dx+block-1)/block,block,0,stream>>>(d_destAddr, &(domain.*src)(0), dx, dx, dy, dz);
         ...
         COMM_SEND_MSG(domain, toRank, 0, 1, 1,
                       d_destAddr, &domain.commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm],
                       xferFields*dx, baseType, msgType,
                       &domain.sendRequest[pmsg+emsg], stream, xferFields, doSend, planeOnly) ;
         ++emsg ;
      }
      ...
      if (colMin && planeMin) {
	    ...
	    SendEdge<2><<<(dy+block-1)/block,block,0,stream>>>(d_destAddr, &(domain.*src)(0), dy, dx, dy, dz);
         ...
         COMM_SEND_MSG(domain, toRank, 1, 0, 1,
                       d_destAddr, &domain.commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm],
                       xferFields*dy, baseType, msgType,
                       &domain.sendRequest[pmsg+emsg], stream, xferFields, doSend, planeOnly) ;
         ++emsg ;
      }
      ...
      if (rowMin && colMin && planeMin) {
         ...
         COMM_SEND_CORNER(domain, toRank, 1, 1, 1, fieldData, 0,
                          &domain.commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm + cmsg * CACHE_COHERENCE_PAD_REAL],
                          &domain.d_commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm + cmsg * CACHE_COHERENCE_PAD_REAL],
                          xferFields, baseType, msgType, &domain.sendRequest[pmsg+emsg+cmsg],
                          stream, doSend, planeOnly) ;
         ++cmsg ;
      }
      if (rowMin && colMin && planeMax && doSend) {
         ...
         COMM_SEND_CORNER(domain, toRank, 1, 1, -1, fieldData, idx,
                          &domain.commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm + cmsg * CACHE_COHERENCE_PAD_REAL],
                          &domain.d_commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm + cmsg * CACHE_COHERENCE_PAD_REAL],
                          xferFields, baseType, msgType, &domain.sendRequest[pmsg+emsg+cmsg],
                          stream, doSend, planeOnly) ;
   ...
   COMM_SEND_FINISH(domain, stream, status) ;
}
...
void CommSBNGpu(Domain& domain, int xferFields, Domain_member *fieldData, cudaStream_t *streams) {
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
         /* contiguous memory */
	 stream = streams[s++];
         srcAddr = (COMM_RECV_BASE(domain) + (pmsg * maxPlaneComm)) ;
         d_srcAddr = &domain.d_commDataRecv[pmsg * maxPlaneComm] ;
         MPI_Wait(&domain.recvRequest[pmsg], &status) ;
         COMM_UNPACK_H2D(d_srcAddr, srcAddr, xferFields*opCount*sizeof(Real_t), stream);
         for (Index_t fi=0 ; fi<xferFields; ++fi) {
   ...
   if (rowMin & colMin) {
      stream = streams[s++];
      srcAddr = (COMM_RECV_BASE(domain) + (pmsg * maxPlaneComm + emsg * maxEdgeComm)) ;
      d_srcAddr = &domain.d_commDataRecv[pmsg * maxPlaneComm +
                                       emsg * maxEdgeComm] ;
      MPI_Wait(&domain.recvRequest[pmsg+emsg], &status) ;
      COMM_UNPACK_H2D(d_srcAddr, srcAddr, xferFields*dz*sizeof(Real_t), stream);
      for (Index_t fi=0 ; fi<xferFields; ++fi) {
         Domain_member dest = fieldData[fi] ;
   ...
   if (rowMin & planeMin) {
      ...
      d_srcAddr = &domain.d_commDataRecv[pmsg * maxPlaneComm +
                                       emsg * maxEdgeComm] ;
      MPI_Wait(&domain.recvRequest[pmsg+emsg], &status) ;
      COMM_UNPACK_H2D(d_srcAddr, srcAddr, xferFields*dx*sizeof(Real_t), stream);
      for (Index_t fi=0 ; fi<xferFields; ++fi) {
         Domain_member dest = fieldData[fi] ;
   ...
   if (colMin & planeMin) {
      ...
      d_srcAddr = &domain.d_commDataRecv[pmsg * maxPlaneComm +
                                       emsg * maxEdgeComm] ;
      MPI_Wait(&domain.recvRequest[pmsg+emsg], &status) ;
      COMM_UNPACK_H2D(d_srcAddr, srcAddr, xferFields*dy*sizeof(Real_t), stream);
      for (Index_t fi=0 ; fi<xferFields; ++fi) {
         Domain_member dest = fieldData[fi] ;
   ...
   if (rowMin & colMin & planeMin) {
      stream = streams[s++];
      /* corner at domain logical coord (0, 0, 0) */
      Real_t *comBuf = (COMM_RECV_BASE(domain) + (pmsg * maxPlaneComm + emsg * maxEdgeComm + cmsg * CACHE_COHERENCE_PAD_REAL)) ;
      MPI_Wait(&domain.recvRequest[pmsg+emsg+cmsg], &status) ;
      for (Index_t fi=0; fi<xferFields; ++fi) {
         COMM_ADD_CORNER(stream, &(domain.*fieldData[fi])(0), comBuf, fi) ;
      }
      ++cmsg ;
   }
   if (rowMin & colMin & planeMax) {
      ...
      MPI_Wait(&domain.recvRequest[pmsg+emsg+cmsg], &status) ;
      for (Index_t fi=0; fi<xferFields; ++fi) {
         COMM_ADD_CORNER(stream, &(domain.*fieldData[fi])(idx), comBuf, fi) ;
      }
      ++cmsg ;
   }
   if (rowMin & colMax & planeMin) {
...
void CommSyncPosVelGpu(Domain& domain, cudaStream_t *streams) {
   ...
   if (rowMin & colMin & planeMin & doRecv) {
      ...
      MPI_Wait(&domain.recvRequest[pmsg+emsg+cmsg], &status) ;
      for (Index_t fi=0; fi<xferFields; ++fi) {
         COMM_COPY_CORNER(stream, &(domain.*fieldData[fi])(0), comBuf, fi) ;
      }
      ++cmsg ;
   }
   if (rowMin & colMin & planeMax) {
      ...
      MPI_Wait(&domain.recvRequest[pmsg+emsg+cmsg], &status) ;
      for (Index_t fi=0; fi<xferFields; ++fi) {
         COMM_COPY_CORNER(stream, &(domain.*fieldData[fi])(idx), comBuf, fi) ;
      }
      ++cmsg ;
   }
...
void CommMonoQGpu(Domain& domain, cudaStream_t stream)
{
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
         ...
         for (Index_t fi=0 ; fi<xferFields; ++fi) {
            Domain_member dest = fieldData[fi] ;
            COMM_MONOQ_COPY(&(domain.*dest)(fieldOffset[fi]), srcAddr, opCount*sizeof(Real_t), stream);
            srcAddr += opCount ;
