...
# printing bandwidth. An earlier GB200 run shipped a bandwidth table produced this way
...
#
# On a GB200 system with HPC-X: `source scripts/env_gb200.sh` in the same shell as BOTH the
...
# One run built against HPC-X 2.22.1 and ran against 2.21: mca_pml_ucx.so
...
# it here instead of after the queue wait. With HPC-X, `source
# scripts/env_gb200.sh` before both the build and this script.
...
# does NOT place one rank per GPU: in an earlier GB200 run it spread 8 ranks over 8
...
# An earlier GB200 run hit exactly this on its sanity check (a stale absolute path) and
